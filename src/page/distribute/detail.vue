<template>
<div class="wy-fix-detail wy-dis-detail">
  ...
  <div class="wy-fix-detItem" v-if="serviceStatus != '1'">
    <div class="wy-fix-det-head">
  ...
  <div id="wy-steps" v-if="serviceStatus != '1'"></div>
</div>
...
export default {
  ...
  watch:{
    ggchooseResult(val){
      if (val && val.name) {
        console.log('val',val)
        this.disrepairersName = val.name;
        this.disrepairersValue = val.id;
      }
    },
    steps(arr){
      ...
      this.$nextTick(()=>{
        if (tmp && tmp.length>0 && this.serviceStatus != '1') {
  ...
  methods:{
    ...
    getListrepairuser(){
      var that = this;
      utils.Get('getListrepairuser',{propertyId:window.userInfo.propertyId}).then(function(res){
        if (res.data.code === 0 && res.data.page &&  res.data.page.list && res.data.page.list.length>0) {
          res.data.page.list.forEach(function(item){
            that.ggslots[0].values.push({
              name:item.ownerName,
              id:item.id
            })
          });
        }else {
          Toast('本小区暂未设置维修工，请联系管理员~')
        }
      });
    },
...
<style scoped>
.wy-dis-detail >>> .mint-cell-allow-right::after {
...
.wy-dis-detail .mint-popup{
  width:100%;
}
.wy-dis-detail .wy-select-problem >>> .mint-cell-title {
  text-align: left;
}
.wy-dis-detail .wy-select-problem >>> .mint-cell {
  background-image: none;
}
.wy-dis-detail .wy-select-problem >>> .mint-cell .mint-cell-wrapper{
background-image: none;
}
.wy-fix-detbuts {
  padding: 16px;
